class UsersController < ApplicationController
  ...
  def get_salt
    User.find_by(username: params[:user][:username]).salt.to_s
  end
  
  def get_pepper
    return 'pepper makes you sneeze :P'
  end
  ...
  def login_attempt
    ...
    respond_to do |format|
      ...
      if User.exists?(username: params[:user][:username], password: encrypted_password) 
  ...
  def create
    ...
    encrypted_password = (Digest::MD5.hexdigest ( @user.salt + get_pepper() + @user['password'].to_s )).to_s
  ...
  private
    ...
    def user_params
      params.require(:user).permit(:firstname, :lastname, :password, :email, :username, :salt)
    end
end